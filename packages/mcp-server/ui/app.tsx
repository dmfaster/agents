...
const tabs: Array<{ id: TabId; label: string; description: string }> = [
  {
    id: "audience",
    label: "Audience",
    description: "Targeting intent and executable lead filters.",
  },
  { id: "messages", label: "Messages", description: "Channel copy, language, and voice." },
  {
    id: "delivery",
    label: "Delivery",
    description: "Channels, schedule, and conservative limits.",
  },
...
function reviewedAudienceFromPreview(value: unknown): ReviewedAudience | null {
  ...
  if (
    ...
    !freshness ||
    Array.isArray(reviewed.dataFreshness) ||
...
function deliveryForecast(state: AgentCampaignState, exactAudience: number | null) {
  ...
  const data: AnalyticsChartDatum[] = Array.from({ length: 21 }, (_, index) => {
    ...
    const planned =
      allowed && cap > 0 ? (remaining === null ? cap : Math.min(cap, Math.max(0, remaining))) : 0;
...
function ChannelIcon({ channel }: { channel: CampaignChannel }) {
  ...
  if (channel === "facebook") {
    return (
      <svg viewBox="0 0 24 24" className="h-4 w-4" aria-hidden="true">
        ...
        <path
          d="M13.25 18v-5.2h1.75l.32-2.05h-2.07V9.42c0-.58.18-.98 1-.98h1.14V6.6c-.55-.08-1.1-.12-1.66-.12-1.65 0-2.78 1.02-2.78 2.86v1.41H9.08v2.05h1.87V18h2.3Z"
          fill="white"
        />
      </svg>
    );
  ...
  if (channel === "linkedin") {
    return (
      <svg viewBox="0 0 24 24" className="h-4 w-4" aria-hidden="true">
        <rect x="4.5" y="4.5" width="15" height="15" rx="3.2" fill="currentColor" />
        <path
          d="M8.15 10.25h2.1v6.1h-2.1v-6.1Zm1.05-2.9a1.18 1.18 0 1 1 0 2.36 1.18 1.18 0 0 1 0-2.36Zm2.35 2.9h2v.82c.32-.5.95-.98 1.98-.98 2.05 0 2.47 1.35 2.47 3.1v3.16h-2.1v-2.8c0-.65-.02-1.48-.9-1.48-.9 0-1.05.7-1.05 1.43v2.85h-2.4v-6.1Z"
          fill="white"
        />
      </svg>
    );
...
function AudienceEditor({
  ...
  const signalTags = state.brief.requestedSignals.map(
    (signal) => `${signal.key.replaceAll("_", " ")} · ${signal.required ? "required" : "optional"}`,
  );
  ...
  return (
    <div className="grid gap-4 sm:grid-cols-2">
      ...
      <Field
        ...
        hint="Describe the audience naturally; the resolved filters below remain authoritative."
      >
        <textarea
          className={`${textareaClass} min-h-28`}
          value={state.brief.targetDescription}
          onChange={(event) =>
            update((next) => {
              next.brief.targetDescription = event.target.value;
            }, true)
          }
        />
      </Field>
      <Field label="Countries">
        <input
          className={inputClass}
          value={state.brief.countries.join(", ")}
          onChange={(event) =>
            update((next) => {
              next.brief.countries = csv(event.target.value).map((value) =>
                value.toUpperCase(),
              ) as typeof next.brief.countries;
            }, true)
          }
        />
      </Field>
      <Field label="Cities">
        <input
          className={inputClass}
          value={(state.brief.cities || []).join(", ")}
          onChange={(event) =>
            update((next) => {
              next.brief.cities = csv(event.target.value);
            }, true)
          }
        />
      </Field>
      <Field label="Decision-maker roles" className="sm:col-span-2">
        <input
          className={inputClass}
          value={state.brief.decisionMakerRoles.join(", ")}
          onChange={(event) =>
            update((next) => {
              next.brief.decisionMakerRoles = csv(event.target.value);
            }, true)
          }
        />
      </Field>
      <Field label="Employees minimum">
        <input
          ...
          type="number"
          min="0"
          value={state.brief.companySize.employeeMin ?? ""}
          onChange={(event) =>
            update((next) => {
              next.brief.companySize.employeeMin = event.target.value
      ...
      <Field label="Employees maximum">
        <input
          ...
          type="number"
          min="0"
          value={state.brief.companySize.employeeMax ?? ""}
          onChange={(event) =>
            update((next) => {
              next.brief.companySize.employeeMax = event.target.value
                ? Number(event.target.value)
                : null;
            }, true)
          }
      ...
      <Field label="Revenue minimum, EUR">
        <input
          ...
          type="number"
          min="0"
          value={state.brief.companySize.revenueMinEur ?? ""}
          onChange={(event) =>
            update((next) => {
              next.brief.companySize.revenueMinEur = event.target.value
                ? Number(event.target.value)
                : null;
            }, true)
          }
      ...
      <Field label="Revenue maximum, EUR">
        <input
          ...
          type="number"
          min="0"
          value={state.brief.companySize.revenueMaxEur ?? ""}
          onChange={(event) =>
            update((next) => {
              next.brief.companySize.revenueMaxEur = event.target.value
                ? Number(event.target.value)
                : null;
            }, true)
          }
      ...
      <Field label="Company exclusions" className="sm:col-span-2">
        <input
          className={inputClass}
          value={state.brief.exclusions.join(", ")}
          onChange={(event) =>
            update((next) => {
              next.brief.exclusions = csv(event.target.value);
            }, true)
          }
        />
      </Field>
      <div className="sm:col-span-2 flex items-center justify-between gap-4 rounded-xl border border-slate-200 bg-white px-4 py-3">
        <div>
          <p className="text-sm font-semibold text-slate-800">
            Exclude previously contacted companies
          </p>
          <p className="mt-1 text-xs leading-5 text-slate-500">
            Use this workspace's contact history so the exact preview and saved list contain only
            new companies.
          </p>
        </div>
        <CampaignToggle
          ...
          onChange={(checked) =>
            update((next) => {
              next.brief.excludePreviouslyContacted = checked;
            }, true)
          }
        />
      </div>
      ...
      <Field label="Call to action" className="sm:col-span-2">
        <input
          className={inputClass}
          value={state.brief.callToAction}
          onChange={(event) =>
            update((next) => {
              next.brief.callToAction = event.target.value;
            })
          }
        />
      </Field>
    ...
    <div className="space-y-4">
      <div className="grid gap-4 sm:grid-cols-2">
        <Field label="Message language">
          <input
            className={inputClass}
            value={state.brief.messageLanguage}
            onChange={(event) =>
              update((next) => {
                next.brief.messageLanguage = event.target.value;
              })
            }
          />
        </Field>
        <Field label="Tone">
          <input
            className={inputClass}
            value={state.brief.tone}
            onChange={(event) =>
              update((next) => {
                next.brief.tone = event.target.value;
                next.profile.preferredTone = event.target.value;
              })
            }
          />
        </Field>
            ...
            <div className="flex items-center justify-between gap-3">
              <div>
                ...
                <p className="mt-0.5 text-xs text-slate-500">
                  {message.origin === "user" ? "Edited by you" : "Agent draft"}
                </p>
              </div>
              <button
                type="button"
                ...
                onClick={() =>
                  update((next) => {
                    next.brief.outreachMessages.splice(messageIndex, 1);
                  })
                }
              >
                Remove
            ...
            <div className="flex flex-wrap gap-2">
              {channels.map((channel) => {
                ...
                return (
                  ...
                  >
                    <span className={selected ? "text-white" : channel.color}>
        ...
        onClick={() =>
          update((next) => {
            next.brief.outreachMessages.push({
              channels: next.brief.requestedChannels.length
                ? [...next.brief.requestedChannels]
                : ["instagram"],
              subject: "",
              body: "",
              origin: "user",
        ...
        <div className="mt-2 grid gap-2 sm:grid-cols-2">
          {channels.map((channel) => {
            ...
            return (
              ...
              >
                <span
                  className={`inline-flex items-center gap-2 text-sm font-semibold text-slate-800 ${channel.color}`}
                >
                  <ChannelIcon channel={channel.id} />
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        <Field label="Daily company limit">
          <input
            ...
            type="number"
            min="1"
            value={state.brief.deliverySettings.dailyCap ?? ""}
            onChange={(event) =>
              update((next) => {
        ...
        <Field label="Timezone">
          <input
            className={inputClass}
            value={state.brief.deliverySettings.timezone}
            onChange={(event) =>
              update((next) => {
                next.brief.deliverySettings.timezone = event.target.value;
              })
            }
          />
        </Field>
        <Field label="Window starts">
          <input
            className={inputClass}
            type="time"
            value={state.brief.deliverySettings.windowStart}
            onChange={(event) =>
              update((next) => {
                next.brief.deliverySettings.windowStart = event.target.value;
              })
            }
          />
        </Field>
        <Field label="Window ends">
          <input
            className={inputClass}
            type="time"
            value={state.brief.deliverySettings.windowEnd}
            onChange={(event) =>
              update((next) => {
                next.brief.deliverySettings.windowEnd = event.target.value;
              })
            }
          />
        </Field>
      ...
      <div className="flex items-center justify-between gap-4 rounded-lg border border-emerald-200 bg-emerald-50 px-3 py-3">
        <div>
          ...
          <p className="mt-0.5 text-xs text-emerald-700">
            Required before a private draft can be prepared.
          </p>
        </div>
        <CampaignToggle
          checked={state.brief.deliverySettings.confirmed}
          onChange={(checked) =>
            update((next) => {
              next.brief.deliverySettings.confirmed = checked;
            })
          }
        />
      </div>
    ...
    <div className="grid gap-4 sm:grid-cols-2">
      <Field label="Business name">
        <input
          ...
          onChange={(event) =>
            update((next) => {
              next.profile.businessName = event.target.value;
            })
          }
        />
      </Field>
      <Field label="Website">
        <input
          className={inputClass}
          inputMode="url"
          value={state.profile.websiteUrl}
          onChange={(event) =>
            update((next) => {
              next.profile.websiteUrl = event.target.value;
            })
          }
        />
      </Field>
      <Field label="Offer" className="sm:col-span-2">
        <textarea
          className={`${textareaClass} min-h-28`}
          value={state.brief.offer}
          onChange={(event) =>
            update((next) => {
              next.brief.offer = event.target.value;
              next.profile.offer = event.target.value;
            })
          }
        />
      </Field>
      <Field label="Customer outcome" className="sm:col-span-2">
        <textarea
          className={`${textareaClass} min-h-28`}
          value={state.profile.customerOutcome}
          onChange={(event) =>
            update((next) => {
              next.profile.customerOutcome = event.target.value;
            })
          }
        />
      </Field>
      <Field label="Business context" className="sm:col-span-2">
        <textarea
          className={`${textareaClass} min-h-32`}
          value={state.profile.businessDescription}
          onChange={(event) =>
            update((next) => {
              next.profile.businessDescription = event.target.value;
            })
          }
        />
      </Field>
      <Field label="Differentiators" className="sm:col-span-2">
        <input
          className={inputClass}
          value={state.profile.differentiators.join(", ")}
          onChange={(event) =>
            update((next) => {
              next.profile.differentiators = csv(event.target.value);
            })
          }
        />
      </Field>
      <Field label="Proof points" className="sm:col-span-2">
        <input
          className={inputClass}
          value={state.profile.proofPoints.join(", ")}
          onChange={(event) =>
            update((next) => {
              next.profile.proofPoints = csv(event.target.value);
            })
          }
        />
      </Field>
      <Field label="Preferred languages">
        <input
          className={inputClass}
          value={state.profile.preferredLanguages.join(", ")}
          onChange={(event) =>
            update((next) => {
              next.profile.preferredLanguages = csv(event.target.value);
            })
          }
        />
      </Field>
      <Field label="Default excluded traits">
        <input
          className={inputClass}
          value={state.profile.excludedCompanyTraits.join(", ")}
          onChange={(event) =>
            update((next) => {
              next.profile.excludedCompanyTraits = csv(event.target.value);
            })
          }
        />
      </Field>
...
function CompanyPreview({ companies }: { companies: PreviewCompany[] }) {
  ...
  return (
    <Panel
      ...
      description={`${companies.length} shown from the exact audience preview.`}
    ...
    >
      <div className="hidden min-h-9 grid-cols-[minmax(220px,1.4fr)_minmax(120px,.7fr)_minmax(150px,1fr)] items-center gap-4 border-b border-slate-200 bg-slate-50 px-4 text-[10px] font-bold uppercase tracking-[0.08em] text-slate-400 md:grid">
        <span className="pl-11">Company</span>
        <span>Location</span>
        <span>Industry</span>
      </div>
      <div className="divide-y divide-slate-200">
        {companies.map((company, index) => {
          ...
          return (
            <div
              key={`${name}-${index}`}
              className="grid min-h-16 items-center gap-3 px-4 py-3 md:grid-cols-[minmax(220px,1.4fr)_minmax(120px,.7fr)_minmax(150px,1fr)] md:gap-4"
            >
              <div className="grid min-w-0 grid-cols-[34px_minmax(0,1fr)] items-center gap-3">
                <span className="grid h-[34px] w-[34px] place-items-center rounded-lg border border-slate-200 bg-slate-50 text-[10px] font-bold text-slate-500">
                  {companyInitials(name)}
                </span>
                <span className="min-w-0">
                  <strong className="block truncate text-xs text-slate-950">{name}</strong>
                  <span className="mt-0.5 block truncate text-[10px] text-slate-400">
                    {company.website || company.domain || "Company preview"}
                  </span>
                </span>
              </div>
...
function App() {
  ...
  const [connection, setConnection] = useState<
    "connecting" | "connected" | "compatibility" | "headless"
  >("connecting");
  ...
  useEffect(() => {
    const handleNotification = (notification: BridgeNotification) => {
      if (notification.method === "ui/notifications/tool-input")
        load(notification.params?.arguments);
  ...
  const update = useCallback(
    (mutator: (next: AgentCampaignState) => void, invalidateAudience = false) => {
      setState((current) => {
  ...
  const forecast = useMemo(
    () => (state ? deliveryForecast(state, exactAudience) : null),
    [exactAudience, state],
  );
  ...
  const consumeResult = useCallback(
    (toolName: string, toolResult: unknown, acceptAudienceReview = true) => {
      ...
      const failed =
        Boolean(asRecord(toolResult)?.isError) ||
        payload.ok === false ||
        ["blocked", "failed", "needs_input"].includes(status);
      const error = asRecord(payload.error);
      const data = asRecord(payload.data);
      const harnessData = asRecord(harness?.data);
      const audience = asRecord(harnessData?.audience);
      const companies = Array.isArray(audience?.companies)
        ? audience.companies
            .filter((company) => asRecord(company))
            .map((company) => company as PreviewCompany)
        : [];
      if (toolName === "audience_preview") {
        const nextReviewedAudience =
          acceptAudienceReview && !failed
      ...
      const setupUrl =
        toolName === "campaign_launch_preflight" && typeof setup?.setupUrl === "string"
          ? setup.setupUrl
          : "";
      const approvalUrl =
        toolName === "campaign_launch_preflight" && typeof data?.approvalUrl === "string"
          ? data.approvalUrl
          : "";
      const summary =
        typeof harness?.summary === "string"
          ? harness.summary
      ...
      const blockers = Array.isArray(harness?.blockers)
        ? harness.blockers.map((blocker) => {
            ...
            return String(
              record?.message || record?.code || "Campaign requirement is not satisfied.",
            );
          })
        : [];
      setResult({
        ...
        approvalUrl,
        confirmationCode:
          typeof authorization?.confirmationCode === "string" ? authorization.confirmationCode : "",
        raw: payload,
      });
    },
    [],
  ...
  const runAction = useCallback(
    async (action: ToolAction) => {
      ...
      try {
        ...
        if (action === "preview") {
          toolName = "audience_preview";
          input = { state, sampleSize: 10 };
        }
        if (action === "prepare") {
          if (!reviewedAudience) {
            throw new Error(
              "Preview and review the exact audience before preparing a private draft.",
            );
          }
    ...
    try {
      ...
      await bridge.updateModelContext({
        content: [
          {
            type: "text",
            text: `${summary} Treat the attached structured campaign state as the latest user-confirmed version.`,
          },
        ],
    ...
    return (
      <main className="min-h-screen bg-slate-50 px-4 py-8 text-slate-950">
        <div className="mx-auto max-w-xl">
          <Panel
            title="Loading campaign workspace"
            description={
              connection === "headless"
                ? "This host received the structured campaign state but does not render MCP Apps."
                : "Waiting for the stateless campaign state from the MCP host."
            }
          >
            <div className="flex items-center gap-3 text-sm text-slate-500">
              <span className="h-2 w-2 animate-pulse rounded-full bg-highlight" />
              DM Faster Campaign Assistant
            </div>
          </Panel>
        </div>
      </main>
      ...
      <div className="mx-auto max-w-[1180px] space-y-3 px-3 py-4 sm:px-5 lg:px-6">
        <PageIntro
          ...
          body={
            ...
            "Review the complete campaign plan, preview the exact audience, and prepare a private draft."
          }
          actions={
            <>
              <TonePill
              ...
              {bridge.context.availableDisplayModes?.includes("fullscreen") ? (
                <button
                  type="button"
                  className={subtleButtonClass}
                  onClick={() => void bridge.toggleDisplayMode()}
                >
                  Expand
                </button>
              ) : null}
        ...
        <div className="grid gap-3 xl:grid-cols-[minmax(0,1.55fr)_minmax(320px,.75fr)] xl:items-start">
          <div className="space-y-3">
            <div
              className="grid grid-cols-4 items-center rounded-xl border border-slate-200 bg-white p-1"
          ...
          <aside className="space-y-3 xl:sticky xl:top-3">
            <Panel
              ...
              description="Validate, preview, and prepare without starting outreach."
            >
              <div className="space-y-3">
                <div className="grid grid-cols-2 gap-2 text-xs">
                  <div className="rounded-lg border border-slate-200 bg-slate-50 px-3 py-2.5">
                    <span className="block text-slate-400">Audience</span>
                    <strong className="mt-1 block text-slate-950">
                      {exactAudience === null
                        ? "Not previewed"
                        : `${formatNumber(exactAudience)} exact`}
                    </strong>
                  </div>
                  <div className="rounded-lg border border-slate-200 bg-slate-50 px-3 py-2.5">
                    <span className="block text-slate-400">Draft</span>
                    <strong className="mt-1 block truncate text-slate-950">
                      {campaignId || "Not created"}
                    </strong>
                  </div>
                  <div className="rounded-lg border border-slate-200 bg-slate-50 px-3 py-2.5">
                    <span className="block text-slate-400">Messages</span>
                    <strong className="mt-1 block text-slate-950">
                      {state.brief.outreachMessages.length}
                    </strong>
                  </div>
                  <div className="rounded-lg border border-slate-200 bg-slate-50 px-3 py-2.5">
                    <span className="block text-slate-400">Schedule</span>
                    <strong className="mt-1 block text-slate-950">
                      {state.brief.deliverySettings.confirmed ? "Confirmed" : "Needs review"}
                    </strong>
                  </div>
                </div>
                ...
                <button
                  ...
                  disabled={!canCall || Boolean(busy) || !reviewedAudience}
                  onClick={() => void runAction("prepare")}
                >
                  {busy === "prepare" ? "Preparing…" : "Prepare private draft"}
                </button>
                <div className="grid grid-cols-2 gap-2">
                  <button
                    type="button"
                    className={`${subtleButtonClass} px-2`}
                    disabled={!canCall || Boolean(busy)}
                ...
                >
                  {busy === "preflight" ? "Requesting…" : "Request launch review"}
                </button>
                <button
                  type="button"
                  className="w-full py-2 text-xs font-bold text-slate-500 transition hover:text-slate-950 disabled:opacity-40"
                  disabled={Boolean(busy) || !dirty}
                  onClick={() => void sync()}
                >
                  {busy === "sync" ? "Syncing…" : "Sync edits to agent"}
                </button>
              </div>
            </Panel>
            ...
            {result ? (
              <Panel
                ...
                action={
                  <TonePill tone={result.failed || result.setupUrl ? "amber" : "green"}>
                ...
                <div className="space-y-3">
                  ...
                  {result.blockers.length ? (
                    <ul className="space-y-2">
                      {result.blockers.map((blocker) => (
                        <li
                          key={blocker}
                          className="rounded-lg border border-red-200 bg-red-50 px-3 py-2 text-xs leading-5 text-red-800"
                        >
                          {blocker}
                        </li>
                      ))}
                  ...
                  {result.setupUrl ? (
                    <div className="rounded-lg border border-blue-200 bg-blue-50 p-3">
                      ...
                      <p className="mt-1 text-xs leading-5 text-blue-800">
                        {result.setupState === "offline"
                          ? "The linked extension is offline. Open its browser and reconnect it, then request launch review again."
                          : "DM Faster will detect this browser, open the correct extension store, and link it to the workspace."}
                      </p>
                      <button
                        type="button"
                        className={`${subtleButtonClass} mt-3 w-full border-blue-300 bg-white`}
                        onClick={() => void bridge.openLink(result.setupUrl)}
                      >
                        Open browser setup
                      </button>
                    </div>
                  ...
                  {result.approvalUrl ? (
                    <div className="rounded-lg border border-amber-200 bg-amber-50 p-3">
                      ...
                      <button
                        type="button"
                        className={`${subtleButtonClass} mt-3 w-full border-amber-300 bg-white`}
                        onClick={() => void bridge.openLink(result.approvalUrl)}
                      >
                        Open approval
                      </button>
                    </div>
                  ...
                  <details className="text-xs text-slate-500">
                    ...
                    <pre className="mt-2 max-h-64 overflow-auto rounded-lg bg-slate-950 p-3 font-mono text-[10px] leading-5 text-slate-100">
